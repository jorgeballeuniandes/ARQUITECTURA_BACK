...
class TestCarrera(TestCase):
    ...
    def test_obtener_carreras_dispnibles_para_apostar(self):
        nueva_carrera = {
            "nombre": self.data_factory.sentence(),
            "competidores": [
                {
        ...
        endpoint = "/carrera/{}/disponible".format(str(self.usuario_code))
        headers = {'Content-Type': 'application/json', "Authorization": "Bearer {}".format(self.token)}

        solicitud_consultar_carreras = self.client.get(endpoint,headers=headers)

        total_carreras_despues = len(json.loads(solicitud_consultar_carreras.get_data()))

        self.assertEqual(solicitud_consultar_carreras.status_code, 200)
